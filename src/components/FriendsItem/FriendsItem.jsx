...
<div className={css.mainInfo}>
    ...
    <div className={css.paragraphDiv}>
        ...
        <p className={css.additionalInfo}><span className={css.additionalInfoSpan}>Email:</span>                        <a href={`mailto:${email}`} className={css.link}>{email}</a>
        </p>
        <p className={css.additionalInfo}><span className={css.additionalInfoSpan}>Address: </span>     <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`} target="_blank" rel="noopener noreferrer" className={css.link}>
                            {address}
                        </a></p>
        <p className={css.additionalInfo}><span className={css.additionalInfoSpan}>Phone: </span>                        <a href={`tel:${phone}`} className={css.link}>{phone}</a>
        </p>
    </div>
